...
<head>
  ...
  <title>CareDesk | Edit Response</title>
  ...
  <div class="flex-1 flex flex-col md:ml-60">
    ...
    <main class="flex-1 p-6 space-y-6">
      ...
      <form action="${pageContext.request.contextPath}/admin/response" method="POST"
        ...
        <div class="space-y-4">
          <div>
            <label for="message" class="block text-sm font-medium mb-1">Response Message</label>
            <textarea id="message" name="message" rows="6" required
                      class="w-full px-4 py-2 rounded-lg border border-orange-300 bg-orange-50 text-gray-800 focus:ring-2 focus:ring-orange-400 focus:outline-none">${response.message}</textarea>
          </div>

          <div>
            <p class="text-sm text-gray-600">
              <strong>Responder:</strong> ${response.responderName} <br>
              <strong>Role:</strong> ${response.responderRole} <br>
              <strong>Created:</strong> ${response.createdAt}
            </p>
          </div>
        </div>
